...
export function useDisableModel(options: UseDisableModelOptions): UseDisableModelReturn {
  ...
  const handleConfirmDisable = useCallback(async () => {
    ...
    try {
      ...
      const targetProvider = targetIndex >= 0 ? providers[targetIndex] : undefined;
      ...
      // 只有当模型确实被过滤掉时才调用 API（官方 API 按 name+index 整条替换）
      if (filteredModels.length < originalModels.length) {
        const updated: OpenAIProviderConfig = { ...targetProvider, models: filteredModels };
        await providersApi.updateOpenAIProvider(
          targetProvider.name,
          targetProvider.sourceIndex ?? targetIndex,
          updated
        );
      }
